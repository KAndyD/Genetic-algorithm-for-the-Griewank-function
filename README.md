...
# 01 · Непрерывная оптимизация

**Генетический алгоритм для функции Гриванка**

`PYTHON` · `ВАРИАНТ 5` · `ВОСПРОИЗВОДИМЫЕ ЭКСПЕРИМЕНТЫ`

**Группа:** 507 · **Номер студента:** 14 · **Год:** 2026 · **Вариант:** 5

![Результат эксперимента](results/convergence_log.png)
...
- [Проверка требований методички](REQUIREMENTS.md)
...
Первая команда — проверка запуска, вторая — полная серия **20 запусков каждого метода**.
...
```bash
python main.py --config config.json --runs 20 --seed 261400 --output my_results
```
...
Содержимое этой папки можно загрузить в корень отдельного репозитория.
Включите `data/`, `results/` и все Markdown-файлы: README ссылается на локальные изображения.
`.venv/` и кэш Python исключены через `.gitignore`. Не загружайте архив вместо распакованного проекта,
если хотите видеть код, графики и README непосредственно в репозитории.
